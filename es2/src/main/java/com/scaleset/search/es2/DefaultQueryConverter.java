...
import com.scaleset.search.es2.agg.*;
import com.scaleset.search.es2.filter.*;
...
public class DefaultQueryConverter implements QueryConverter {
    ...
    protected void addFilters(BoolQueryBuilder boolFilter) {
        Map<String, Filter> filters = query.getFilters();
        if (filters != null) {
            for (Filter filter : filters.values()) {
                QueryBuilder filterBuilder = converterFilter(filter);
    ...
    @Override
    public QueryBuilder converterFilter(Filter filter) {
    ...
    protected void addBbox(BoolQueryBuilder boolFilter) {
        Envelope bbox = query.getBbox();
        String geoField = query.getGeoField();
        if ((geoField != null && !geoField.isEmpty()) && (bbox != null)) {
            boolFilter.must(geoBoundingBoxQuery(geoField).bottomLeft(bbox.getMinY(), bbox.getMinX()).topRight(bbox.getMaxY(), bbox.getMaxX()));
        }
    }

    protected void setFilter(SearchRequestBuilder builder, BoolQueryBuilder boolFilter) {
    ...
    public SearchRequestBuilder searchRequest() {
        ...
        BoolQueryBuilder boolFilter = boolQuery();
        addPaging(builder);
